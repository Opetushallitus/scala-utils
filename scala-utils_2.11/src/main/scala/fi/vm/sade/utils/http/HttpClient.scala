...
object DefaultHttpClient extends HttpClient {
  ...
  def httpPost(url: String, data: Option[String], options: HttpOptions.HttpOption*)(clientCallerId: String): HttpRequest = {
    ...
    data match {
      case None => new DefaultHttpRequest(postRequest.postForm.header("Caller-Id", clientCallerId).header("CSRF", clientCallerId))
      case Some(data) => new DefaultHttpRequest(postRequest.postData(data).header("Caller-Id", clientCallerId).header("CSRF", clientCallerId))
    }
  }
  ...
  def httpPut(url: String, options: HttpOptions.HttpOption*)(clientCallerId: String) : HttpRequest = {
    new DefaultHttpRequest(Http(url).method("PUT").options(options).header("Caller-Id", clientCallerId).header("CSRF", clientCallerId))
  }
