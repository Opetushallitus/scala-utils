...
class CasAbleClient(casClient: CasClient, casParams: CasParams, client: Client) extends Client {
   ...
   override def prepare(req: Request): Task[Response] = {
     ...
     def requestProcess(req:Request): Process[Task, Response] = Process(req).toSource zip(sessions) through requestChannel flatMap {
