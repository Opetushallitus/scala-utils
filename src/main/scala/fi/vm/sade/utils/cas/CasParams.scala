...
object CasParams {
  ...
  def apply(service:String, username: String, password: String):CasParams =
    Uri.fromString(ensureTailingSlash(service)).fold((e) => throw ParseException(e), (service: Uri) => CasParams(CasService(resolve(service, uri("j_spring_cas_security_check"))), CasUser(username, password)))

  def ensureTailingSlash(service:String) = service.last match {
    case '/' => service
    case _ => service + "/"
  }
}
